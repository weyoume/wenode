...
target_link_libraries( {plugin_provider}_{plugin_name} node_app node_chain node_protocol )
...
{{
   public:
      {plugin_name}_api( const node::app::api_context& ctx );
...
class {plugin_name}_plugin : public node::app::plugin
{{
   public:
      {plugin_name}_plugin( node::app::application* app );
...
{{
   public:
      {plugin_name}_api_impl( node::app::application& _app );
...
{plugin_name}_api_impl::{plugin_name}_api_impl( node::app::application& _app ) : app( _app )
{{}}
...
{plugin_name}_api::{plugin_name}_api( const node::app::api_context& ctx )
{{
...
{{
   public:
      {plugin_name}_plugin_impl( node::app::application& app );
...
{plugin_name}_plugin_impl::{plugin_name}_plugin_impl( node::app::application& app )
  : _app(app) {{}}
...
{plugin_name}_plugin::{plugin_name}_plugin( node::app::application* app )
   : plugin(app)
{{
...
DEFINE_PLUGIN( {plugin_name}, {plugin_provider}::plugin::{plugin_name}::{plugin_name}_plugin )
""",
}
