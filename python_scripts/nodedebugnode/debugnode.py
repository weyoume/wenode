...
class DebugNode( object ):
   ...
   def __init__( self, node, data_dir, args='', plugins=[], apis=[], node_out=None, node_err=None ):
      ...
      args:
         node -- The string path to the location of the node binary
         data_dir -- The string path to an existing node data directory which will be used to pull blocks from.
         args -- Other string args to pass to node.
         ...
         node_stdout -- A stream for node's stdout. Default is to pipe to /dev/null
         node_stderr -- A stream for node's stderr. Default is to pipe to /dev/null
      """
      self._data_dir = None
      ...
      self._node_bin = Path( node )
      if( not self._node_bin.exists() ):
         raise ValueError( 'node does not exist' )
      if( not self._node_bin.is_file() ):
      ...
      if( node_out != None ):
         ...
         self.node_out = self._FNULL

      if( node_err != None ):
         self.node_err = node_err
      else:
         self.node_err = self._FNULL
   ...
   def __enter__( self ):
      ...
      node = [ str( self._node_bin ), '--data-dir=' + str( self._temp_data_dir.name ) ]
      node.extend( self._args )

      self._node_process = Popen( node, stdout=self.node_out, stderr=self.node_err )
      self._node_process.poll()
      sleep( 5 )
      if( not self._node_process.returncode ):
         self._rpc = SteemNodeRPC( 'ws://127.0.0.1:8095', '', '' )
      else:
         raise Exception( "node did not start properly..." )

   def __exit__( self, exc, value, tb ):
      self._rpc = None

      if( self._node_process != None ):
         self._node_process.poll()

         if( not self._node_process.returncode ):
            self._node_process.send_signal( SIGINT )

            sleep( 7 )
            self._node_process.poll()

            if( not self._node_process.returncode ):
               self._node_process.send_signal( SIGTERM )

               sleep( 5 )
               self._node_process.poll()

               if( self._node_process.returncode ):
                  loggin.error( 'node did not properly shut down after SIGINT and SIGTERM. User intervention may be required.' )
   ...
   def debug_set_hardfork( self, hardfork_id ):
      ...
      args:
         hardfork_id: The id of the hardfork to set. Hardfork IDs start at 1 (0 is genesis) and increment
            by one for each hardfork. The maximum value is NUM_HARDFORKS in chain/hardfork.d/0-preamble.hf
      """
      if( hardfork_id < 0 ):
...
if __name__=="__main__":
   ...
   def main():
      global WAITING
      """
      This example contains a simple parser to obtain the locations of both node and the data directory,
      ...
      debug_node = DebugNode( str( node ), str( data_dir ), node_err=sys.stderr )
