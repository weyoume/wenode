...
/*BOOST_AUTO_TEST_CASE( comment_payout )
{
   try
   {
      ...
      auto bob_comment_payout = asset( ( ( uint128_t( bob_comment_rshares.value ) * bob_comment_rshares.value * reward_steem.amount.value ) / total_rshares2 ).to_uint64(), EZIRA_SYMBOL );
...
BOOST_AUTO_TEST_CASE( steem_inflation )
{
   ...
   /*
      ...
      auto new_vesting_steem = asset( 0, EZIRA_SYMBOL );
      ...
      while( db.head_block_num() < EZIRA_START_VESTING_BLOCK - 1)
      {
         ...
         new_vesting_steem = asset( 0, EZIRA_SYMBOL );
      ...
      while( db.head_block_num() < EZIRA_START_MINER_VOTING_BLOCK - 1 )
      {
         ...
         witness_pay = std::max( EZIRA_MIN_PRODUCER_REWARD, asset( ( EZIRA_PRODUCER_APR * gpo.virtual_supply.amount ) / ( EZIRA_BLOCKS_PER_YEAR * 100 ), EZIRA_SYMBOL ) );
         auto witness_pay_shares = asset( 0, VESTS_SYMBOL );
         new_vesting_steem = asset( ( witness_pay + new_rewards ).amount * 9, EZIRA_SYMBOL );
         new_vesting_shares = gpo.total_vesting_shares;
      ...
      for( int i = 0; i < EZIRA_BLOCKS_PER_DAY; i++ )
      {
         ...
         witness_pay = std::max( EZIRA_MIN_PRODUCER_REWARD, asset( ( EZIRA_PRODUCER_APR * gpo.virtual_supply.amount ) / ( EZIRA_BLOCKS_PER_YEAR * 100 ), EZIRA_SYMBOL ) );
         witness_pay_shares = witness_pay * gpo.get_vesting_share_price();
         new_vesting_steem = asset( ( witness_pay + new_rewards ).amount * 9, EZIRA_SYMBOL ) + witness_pay;
      ...
      BOOST_REQUIRE_EQUAL( gpo.total_reward_fund_steem.amount.value,
         reward_steem.amount.value + virtual_supply.amount.value / 10 / EZIRA_BLOCKS_PER_YEAR + virtual_supply.amount.value / 10 / EZIRA_BLOCKS_PER_DAY );
      BOOST_REQUIRE_EQUAL( db.get_account( witness_name ).vesting_shares.amount.value,
         old_witness_shares.amount.value + ( asset( ( ( virtual_supply.amount.value / EZIRA_BLOCKS_PER_YEAR ) * EZIRA_1_PERCENT ) / EZIRA_100_PERCENT, EZIRA_SYMBOL ) * ( vesting_shares / vesting_steem ) ).amount.value );
      validate_database();
      */
   }
   FC_LOG_AND_RETHROW();
...
BOOST_AUTO_TEST_CASE( sbd_stability )
{
   try
   {
      ...
      auto comment_reward = ( gpo.total_reward_fund_steem.amount + 2000 ) - ( ( gpo.total_reward_fund_steem.amount + 2000 ) * 25 * EZIRA_1_PERCENT ) / EZIRA_100_PERCENT ;
      comment_reward /= 2;
...
BOOST_AUTO_TEST_CASE( clear_null_account )
{
   try
   {
      ...
      BOOST_REQUIRE( db.get_account( EZIRA_NULL_ACCOUNT ).reward_steem_balance == ASSET( "1.000 TESTS" ) );
      BOOST_REQUIRE( db.get_account( EZIRA_NULL_ACCOUNT ).reward_vesting_balance == ASSET( "1.000000 VESTS" ) );
      BOOST_REQUIRE( db.get_account( EZIRA_NULL_ACCOUNT ).reward_vesting_steem == ASSET( "1.000 TESTS" ) );
      ...
      BOOST_REQUIRE( db.get_account( EZIRA_NULL_ACCOUNT ).reward_steem_balance == ASSET( "0.000 TESTS" ) );
      BOOST_REQUIRE( db.get_account( EZIRA_NULL_ACCOUNT ).reward_vesting_balance == ASSET( "0.000000 VESTS" ) );
      BOOST_REQUIRE( db.get_account( EZIRA_NULL_ACCOUNT ).reward_vesting_steem == ASSET( "0.000 TESTS" ) );
      BOOST_REQUIRE( db.get_account( "alice" ).balance == ASSET( "2.000 TESTS" ) );
      BOOST_REQUIRE( db.get_account( "alice" ).sbd_balance == ASSET( "3.000 TBD" ) );
   }
   FC_LOG_AND_RETHROW()
