...
class hello_api_plugin : public eznode::app::plugin
{
   public:
       ...
       */
      hello_api_plugin( eznode::app::application* app );
...
{
   public:
      hello_api_api( const eznode::app::api_context& ctx );
...
FC_API( eznode::example_plugin::hello_api_api,
   (get_message)
   )
...
hello_api_api::hello_api_api( const eznode::app::api_context& ctx ) : _app(ctx.app) {}
...
/**
 * The DEFINE_PLUGIN() macro will define a eznode::plugin::create_hello_api_plugin()
 * factory method which is expected by the manifest.
 */

DEFINE_PLUGIN( hello_api, eznode::example_plugin::hello_api_plugin )
