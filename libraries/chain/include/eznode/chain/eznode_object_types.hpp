...
using eznode::protocol::chain_id_type;
...
{
   class variant;
   inline void to_variant( const eznode::chain::shared_string& s, variant& var )
   {
      var = fc::string( eznode::chain::to_string( s ) );
   }

   inline void from_variant( const variant& var, eznode::chain::shared_string& s )
   {
   ...
   namespace raw
   {
      ...
      template< typename T > inline void pack( eznode::chain::buffer_type& raw, const T& v )
      {
      ...
      template< typename T > inline void unpack( const eznode::chain::buffer_type& raw, T& v )
      {
         datastream< const char* > ds( raw.data(), raw.size() );
         unpack( ds, v );
      }

      template< typename T > inline T unpack( const eznode::chain::buffer_type& raw )
      {
         T v;
         datastream< const char* > ds( raw.data(), raw.size() );
         unpack( ds, v );
...
FC_REFLECT_ENUM( eznode::chain::object_type,
                 (dynamic_global_property_object_type)
                 (account_object_type)
                 (account_authority_object_type)
                 (witness_object_type)
...
FC_REFLECT_TYPENAME( eznode::chain::buffer_type )

FC_REFLECT_ENUM( eznode::chain::bandwidth_type, (post)(forum)(market) )
