...
{
   public:
      auth_util_api( const eznode::app::api_context& ctx );
...
FC_REFLECT( eznode::plugin::auth_util::check_authority_signature_params,
   (account_name)
   (level)
   (dig)
   (sigs)
   )
FC_REFLECT( eznode::plugin::auth_util::check_authority_signature_result,
   (keys)
   )

FC_API( eznode::plugin::auth_util::auth_util_api,
