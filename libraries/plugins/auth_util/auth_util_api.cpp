...
{
   public:
      auth_util_api_impl( node::app::application& _app );
      void check_authority_signature( const check_authority_signature_params& args, check_authority_signature_result& result );

      std::shared_ptr< node::plugin::auth_util::auth_util_plugin > get_plugin();

      node::app::application& app;
};

auth_util_api_impl::auth_util_api_impl( node::app::application& _app ) : app( _app )
{}

std::shared_ptr< node::plugin::auth_util::auth_util_plugin > auth_util_api_impl::get_plugin()
{
...
void auth_util_api_impl::check_authority_signature( const check_authority_signature_params& args, check_authority_signature_result& result )
{
   ...
   {
      auth = protocol::authority( acct.posting_auth );
   }
   else if( (args.level == "active") || (args.level == "a") || (args.level == "") )
   {
      auth = protocol::authority( acct.active_auth );
   }
   else if( (args.level == "owner") || (args.level == "o") )
   {
      auth = protocol::authority( acct.owner_auth );
   }
   else
   {
   ...
   {
      return protocol::authority(db->get< chain::account_authority_object, chain::by_account >( account_name ).active_auth );
   }, avail );
...
auth_util_api::auth_util_api( const node::app::api_context& ctx )
{
