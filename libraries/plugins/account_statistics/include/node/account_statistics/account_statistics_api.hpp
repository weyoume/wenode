#pragma once

#include <node/account_statistics/account_statistics_plugin.hpp>

#include <fc/api.hpp>

namespace node{ namespace app {
   struct api_context;
} }

namespace node { namespace account_statistics {

namespace detail
{
   class account_statistics_api_impl;
}

class account_statistics_api
{
   public:
      account_statistics_api( const node::app::api_context& ctx );

      void on_api_startup();

   private:
      std::shared_ptr< detail::account_statistics_api_impl > _my;
};

} } // node::account_statistics

FC_API( node::account_statistics::account_statistics_api, )