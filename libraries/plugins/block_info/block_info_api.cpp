...
{
   public:
      block_info_api_impl( node::app::application& _app );

      std::shared_ptr< node::plugin::block_info::block_info_plugin > get_plugin();
...
block_info_api_impl::block_info_api_impl( node::app::application& _app ) : app( _app )
{}

std::shared_ptr< node::plugin::block_info::block_info_plugin > block_info_api_impl::get_plugin()
{
...
void block_info_api_impl::get_block_info( const get_block_info_args& args, std::vector< block_info >& result )
{
   ...
   for( uint64_t block_num=args.start_block_num; block_num<n; block_num++ )
...
block_info_api::block_info_api( const node::app::api_context& ctx )
{
