...
namespace ezira { namespace protocol {
   ...
   struct asset
   {
      asset( share_type a = 0, asset_symbol_type id = SYMBOL_EZIRA )
      :amount(a),symbol(id){}
...
namespace fc {
    inline void to_variant( const ezira::protocol::asset& var,  fc::variant& vo ) { vo = var.to_string(); }
    inline void from_variant( const fc::variant& var,  ezira::protocol::asset& vo ) { vo = ezira::protocol::asset::from_string( var.as_string() ); }
}

FC_REFLECT( ezira::protocol::asset, (amount)(symbol) )
FC_REFLECT( ezira::protocol::price, (base)(quote) )
