...
namespace node { namespace protocol {
   ...
   struct asset
   {
      asset( share_type a = 0, asset_symbol_type id = SYMBOL_COIN )
      :amount(a),symbol(id){}
...
namespace fc {
    inline void to_variant( const node::protocol::asset& var,  fc::variant& vo ) { vo = var.to_string(); }
    inline void from_variant( const fc::variant& var,  node::protocol::asset& vo ) { vo = node::protocol::asset::from_string( var.as_string() ); }
}

FC_REFLECT( node::protocol::asset, (amount)(symbol) )
FC_REFLECT( node::protocol::price, (base)(quote) )
