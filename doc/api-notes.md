...
The WeYouMe developers recommend using the first of these methods to secure the API by binding to localhost, as follows:
...
The `node` process includes username/password based authentication to individual API's.
...
`api-user` definition to define the access policy:
    ...
    enable-plugin = producer account_history hello_api
